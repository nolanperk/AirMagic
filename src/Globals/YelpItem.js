import React, { Component } from 'react';
import propTypes from 'prop-types';
import axios from 'axios';
import YelpAPI from '../yelp'
import ApiConfig from '../config'

import loader from '../assets/loader.gif';
import exit from '../assets/icons/white/exit.png';
import mapLink from '../assets/icons/black/location.png';
import popout from '../assets/icons/popout.png';

export default class YelpItem extends Component {
  constructor(props) {
    super();
    this.state = {
      loading: true,
    }
  }


  scrubCheck = (e) => {
    let companyName = e.name;
    let address = e.location.address1;
    let phone = e.display_phone;

    let customersId;
    let salesId;
    if (this.props.yelpRegion === 'tampa') {
      customersId = 'apps7GoAgK23yrOoY';
      salesId = 'appEX8GXgcD2ln4dB';
    } else if (this.props.yelpRegion === 'orlando') {
      customersId = 'appBUKBn552B8SlbE';
      salesId = 'appXNufXR9nQARjgs';
    }


    let finalURL = '';
    let activesProp = {
      company: {
        sales: [],
        customers: [],
      },
      phone: {
        sales: [],
        customers: [],
      },
      address: {
        sales: [],
        customers: [],
      },
    };

    let phoneCheck = function() {
      let propedArr = [];

      finalURL = 'https://api.airtable.com/v0/' + customersId + '/Customers?view=All+Actives';
      finalURL += '&filterByFormula=(' + 'IF(%7BOffice+Phone%7D+%3D+"' + phone + '"%2CTRUE()))';

      return axios
        .get(finalURL).then(response => {
          if (response.data.records.length > 0) {
            let theseRecs = propedArr.concat(response.data.records);
            activesProp.phone.customers = theseRecs;
          }

          finalURL = 'https://api.airtable.com/v0/' + salesId + '/Sales?view=All';
          finalURL += '&filterByFormula=(' + 'IF(%7BOffice+Phone%7D+%3D+"' + phone + '"%2CTRUE()))';
          return axios
            .get(finalURL).then(response => {
              if (response.data.records.length > 0) {
                let theseRecs = propedArr.concat(response.data.records);
                activesProp.phone.sales = theseRecs;
              }

              addrCheck();
            }).catch(error => {
              this.setState({loading:false});
            });
        }).catch(error => {
          this.setState({loading:false});
        });
    }.bind(this);
    phoneCheck();


    let addrCheck = function() {
      let propedArr = [];

      finalURL = 'https://api.airtable.com/v0/' + customersId + '/Customers?view=All+Actives';
      finalURL += '&filterByFormula=(' + 'IF(LOWER(%7BAddress+1%7D)+%3D+LOWER("' + address + '")%2CTRUE()))';

      return axios
        .get(finalURL).then(response => {
          if (response.data.records.length > 0) {
            let theseRecs = propedArr.concat(response.data.records);
            activesProp.address.customers = theseRecs;
          }

            finalURL = 'https://api.airtable.com/v0/' + salesId + '/Sales?view=All';
            finalURL += '&filterByFormula=(' + 'IF(LOWER(%7BAddress+1%7D)+%3D+LOWER("' + address + '")%2CTRUE()))';
            return axios
              .get(finalURL).then(response => {
                if (response.data.records.length > 0) {
                  let theseRecs = propedArr.concat(response.data.records);
                  activesProp.address.sales = theseRecs;
                }
                compCheck();
            }).catch(error => {
              this.setState({loading:false});
            });
        }).catch(error => {
          this.setState({loading:false});
        });
    }.bind(this);


    let compCheck = function() {
      let propedArr = [];

      finalURL = 'https://api.airtable.com/v0/' + customersId + '/Customers?view=All+Actives';
      finalURL += '&filterByFormula=(' + 'IF(LOWER(%7BCompany+Name%7D)+%3D+LOWER("' + companyName + '")%2CTRUE()))';

      return axios
        .get(finalURL).then(response => {
          if (response.data.records.length > 0) {
            let theseRecs = propedArr.concat(response.data.records);
            activesProp.company.customers = theseRecs;
          }

          finalURL = 'https://api.airtable.com/v0/' + salesId + '/Sales?view=All';
          finalURL += '&filterByFormula=(' + 'IF(LOWER(%7BCompany+Name%7D)+%3D+LOWER("' + companyName + '")%2CTRUE()))';
          return axios
            .get(finalURL).then(response => {
              if (response.data.records.length > 0) {
                let theseRecs = propedArr.concat(response.data.records);
                activesProp.company.sales = theseRecs;
              }
              finishChecks();
            }).catch(error => {
              this.setState({loading:false});
            });
        }).catch(error => {
          this.setState({loading:false});
        });
    }.bind(this);

    let finishChecks = function() {
      if (
        activesProp.company.customers.length > 0 || activesProp.phone.customers.length > 0 || activesProp.address.customers.length > 0 ||
        activesProp.company.sales.length > 0 || activesProp.phone.sales.length > 0 || activesProp.address.sales.length > 0
      ) {
        this.setState({
          hasActive: true
        });
      }
      this.setState({
        loading: false,
        actives: activesProp
      })
      console.log(activesProp);
    }.bind(this);
  }

  saveToDB = (e, d) => {
    console.log(e);
    document.getElementsByClassName(e)[0].className += ' saved';
    this.props.addToDB(d);
  }

  unHide = (e) => {
    if (document.getElementsByClassName(e)[0].className.includes("hideIt")) {
      document.getElementsByClassName(e)[0].className = document.getElementsByClassName(e)[0].className.replace(' hideIt', '');
    }
  }

  removeItem = (e) => {
    document.getElementsByClassName(e)[0].className += ' hideIt';
  }


  componentDidMount() {
    this.scrubCheck(this.props.data);
  }
  // Render
  // ----------------------------------------------------
  render() {

    let selectClass = 'selectFields';
    if (this.state.keywordFilter === 'select') {
      selectClass += ' active';
    }
    let searchClass = 'searchFields';
    if (this.state.keywordFilter === 'search') {
      searchClass += ' active';
    }
    let key = `${this.props.data.alias}`;
    let googleUrl = 'https://www.google.com/search?q=' + this.props.data.name.replace(/ /g, '+');

    if (this.state.actives) {
      if (this.state.actives.phone.sales.length > 0 || this.state.actives.phone.customers.length > 0 || this.state.actives.company.sales.length > 0 || this.state.actives.company.customers.length > 0) {
        key += ' red';
      }
    }
    return (
      <li key={key} className={key}>
        <div className="imgArea">
          {this.duplicateCheck}
          <a className="popOut" target="_blank" href={this.props.data.url}><img src='https://s3-media0.fl.yelpcdn.com/assets/public/default@2x.yelp_design_web.yji-a536dc4612adf182807e56e390709483.png' alt="View on Yelp" /></a>
          <a className="mapOut" target="_blank"
            href={'https://www.google.com/maps/search/?api=1&query=' + encodeURI(this.props.data.name + ' ' + this.props.data.location.display_address.map((e, i) => {
              return (
                e + ' '
              )
            }))}>
            <img src={mapLink} alt="View on Map" />
          </a>
          <a className="googleOut" target="_blank" href={googleUrl}><img src='data:image/png;base64,' alt="View on Google" /></a>
          <img src={this.props.data.image_url} alt={this.props.data.name} />
        </div>
        <div className="textArea">
          <h2>{this.props.data.name}</h2>
          <p>Reviews: {this.props.data.review_count}</p>
          <p>{this.props.data.location.display_address.map((e, i) => {
            return (
              e + ' '
            )
          })}</p>
          <div className="softGrad--black btn" onClick={()=>this.saveToDB(key, this.props.data)}>Save</div>
          <div className="softGrad--black btn removeBtn" onClick={()=>this.removeItem(key)}>Remove</div>
        </div>

      </li>
    );
  }

  get duplicateCheck() {
    if (this.state.loading) {
      return (
        <img className="loadGif" src={loader} />
      )
    } else if (this.state.hasActive) {
      let dupeClass = 'dupeDot orange';

      if (this.state.actives.phone.sales.length > 0 || this.state.actives.phone.customers.length > 0 || this.state.actives.company.sales.length > 0 || this.state.actives.company.customers.length > 0) {
        dupeClass = 'dupeDot red';
      }
      return (
        <div className={dupeClass}>
          <div className="inner">
            {this.state.actives.company.sales.length > 0 ? this.duplicateItem(this.state.actives.company.sales[0], 'name') : ''}
            {this.state.actives.company.customers.length > 0 ? this.duplicateItem(this.state.actives.company.customers[0], 'name') : ''}

            {this.state.actives.phone.sales.length > 0 ? this.duplicateItem(this.state.actives.phone.sales[0], 'phone') : ''}
            {this.state.actives.phone.customers.length > 0 ? this.duplicateItem(this.state.actives.phone.customers[0], 'phone') : ''}

            {this.state.actives.address.sales.length > 0 ? this.duplicateItem(this.state.actives.address.sales[0], 'address') : ''}
            {this.state.actives.address.customers.length > 0 ? this.duplicateItem(this.state.actives.address.customers[0], 'address') : ''}
          </div>
        </div>
      )
    } else {
      return (
        <div className="dupeDot green"></div>
      )
    }
  }

  duplicateItem(item, type) {
    let googleUrl = 'https://www.google.com/search?q=' + item.fields['Company Name'].replace(/ /g, '+') + '+' + this.props.data.location.address1.replace(/ /g, '+');
    if (type === 'address') {
      return (
        <div className="dupeItem">
          <a href={googleUrl} target="_blank">
            <p>Same {type}</p>
            <h2>{item.fields['Company Name']}</h2>
          </a>
        </div>
      )
    } else {
      return (
        <div className="dupeItem">
          <p>Same {type}</p>
          <h2>{item.fields['Company Name']}</h2>
        </div>
      )
    }
  }

}
