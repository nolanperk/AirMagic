...
export default class CustomerItem extends Component {
  ...
  render() {
    ...
    return (
      <tr key={this.props.id} className="markerRow" id={this.props.id} index={this.props.index} customersData={customersData} onClick={()=>this.props.markerClicked(customersData, this.props.id, this.props.position)}>
        <td></td>
        ...
        <td><a href={'/' + this.props.citySet + '/customer-service/' + this.props.id.replace('row-', '')} target="_blank">View</a></td>
        <td></td>
      </tr>
    );
  }
